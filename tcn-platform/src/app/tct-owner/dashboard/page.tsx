...
export default function TctOwnerDashboard() {
    ...
    <div style={{ position: 'relative', zIndex: 10 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, color: '#5452d6', fontFamily: 'var(--font-dm)', fontWeight: 500 }}>
          <strong>TCT Owner — View-only access.</strong> You can browse the registry. To signal interest in a listing, you must become a TCT Participant by electing to participate and completing fund transfer acknowledgment.
        </span>
      </div>
      ...
      >
        ...
        <p style={{ fontSize: 14, color: '#7a78a8', lineHeight: 1.85, maxWidth: 500 }}>
          Browse verified collaboration opportunities. As a <strong style={{ color: '#2e2c5e', fontWeight: 600 }}>TCT Owner</strong>, you have view-only access. Signaling interest requires TCT Participant status.
        </p>
        ...
        <div className="flex gap-8 mt-8">
          {[
            ...
            { label: 'Your Access Level', value: 'Owner', highlight: true },
          ].map((stat) => (
            <div key={stat.label}>
              <div style={{ fontFamily: 'var(--font-outfit)', fontSize: 28, fontWeight: 800, color: stat.highlight ? '#7a78a8' : '#5452d6', letterSpacing: '-0.02em' }}>
                {stat.value}
              </div>
      ...
      <div style={{ padding: '36px 60px 80px' }}>
        ...
        <div className="flex items-center justify-between mb-5 flex-wrap gap-4">
          <div className="flex items-center gap-2 flex-wrap">
            <span style={{ fontSize: 11, fontWeight: 700, color: '#7a78a8', letterSpacing: '0.07em', textTransform: 'uppercase', marginRight: 4 }}>SECTOR</span>
            {sectors.map((s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
        ...
        <div className="flex items-center gap-2 mb-5">
          <span style={{ fontFamily: 'var(--font-outfit)', fontSize: 18, fontWeight: 800, color: '#0f0e2a', letterSpacing: '-0.01em' }}>Active Listings</span>
        ...
        <div style={{
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tr>
                  {['Project', 'Sector', 'Location', 'Collaboration Value', 'Timeline', 'Signals', ''].map((h) => (
                    <th key={h} style={{
                      ...
                      color: 'rgba(255,255,255,0.5)', textAlign: 'left', whiteSpace: 'nowrap',
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((listing, i) => (
                  <tr
                    key={listing.id}
                    onClick={() => router.push(`/tct-owner/listings/${listing.id}`)}
                    style={{
                  ...
                  >
                    <td style={{ padding: '20px' }}>
                      <div style={{ fontFamily: 'var(--font-dm)', fontSize: 14, fontWeight: 700, color: '#0f0e2a', lineHeight: 1.3, marginBottom: 4 }}>{listing.name}</div>
                    ...
                    <td style={{ padding: '20px', fontSize: 13, color: '#2e2c5e' }}>{listing.location}</td>
                    <td style={{ padding: '20px', fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 600, color: '#0f0e2a' }}>{formatAmount(listing.targetAmount)}</td>
                    <td style={{ padding: '20px', fontSize: 13, color: '#7a78a8' }}>{listing.timeline}</td>
                    <td style={{ padding: '20px', fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 600, color: '#5452d6', textAlign: 'center' }}>{listing.signalCount}</td>
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ padding: '48px', textAlign: 'center', color: '#7a78a8', fontSize: 14 }}>No listings match your filter.</div>
          )}
        </div>
      </div>
    </div>
